...
export class YtrawlPanel {
    ...
    _buildAdvancedBody() {
        ...
        const resetBtn = UI.ghostButton({
            ...
            onActivate: () => this.c._deps.resetSettings && this.c._deps.resetSettings(),
        });
        add(resetBtn);
    ...
    _render() {
        ...
        this._extractBtn.visible = noInfo && notDownloading;
    ...
    _rebuildHistory(history) {
        ...
        for (const url of shown) {
            const btn = UI.stWidget(St.Button, {
                label: url,
                style_class: 'ytrawl-history-item',
                x_expand: true,
                can_focus: true,
            });
