...
export function accordion({ titleText, valueText = '', content, defaultOpen = false, onToggleOpen }) {
    ...
    // caption() ellipsizes its ClutterText, and an ellipsized label can get
    // allocated well below its natural text width by the box layout (it
    // can always fall back to "…"), even with x_expand off and plenty of
    // room in the row. A floor width keeps short values like "Off" or
    // "Nothing" from truncating unnecessarily.
    vLabel.add_style_class_name('ytrawl-accordion-value');
